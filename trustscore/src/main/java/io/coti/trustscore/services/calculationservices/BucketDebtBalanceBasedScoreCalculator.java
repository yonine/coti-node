...
public class BucketDebtBalanceBasedScoreCalculator extends BucketScoresCalculator<BucketDebtBalanceBasedEventData> {
    private static final double ACCRUEMENT_FACTOR = 1.0/ 365.0;
    private BalanceBasedUserParameters debtUserParameters;

    public BucketDebtBalanceBasedScoreCalculator(BucketDebtBalanceBasedEventData bucketData) {
        super(bucketData);
        debtUserParameters = (BalanceBasedUserParameters) BucketDebtBalanceBasedScoreService.userParameters(
                FinalEventType.DEBT, bucketData.getUserType());
    }
